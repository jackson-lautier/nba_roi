#new contrib formula for WS
total_contrib_WS_ROI = function(player_name){
  ...
  if(length(player_team) == 1){
    ...
    for(g in player_games){
      ...
      if( length(play) == 1){
        player_contribs = append(player_contribs, df$game_WS[play])
      }
  ...
  if(length(player_team) > 1){
    ...
    for(t in player_team){
      ...
      for(g in player_games_t){
        ...
        if( length(play) == 1){
          t_contribs = append(t_contribs, df$game_WS[play])
        }
        else if( length(play) == 0){

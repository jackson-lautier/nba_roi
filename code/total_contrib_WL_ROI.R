#different version to get game ids
total_contrib_WL_ROI = function(player_name){
  ...
  if(length(player_team) == 1){
    ...
    for(g in player_games){
      ...
      if( length(play) == 1){
        player_contribs = append(player_contribs, df$win_logit[play])
      }
  ...
  if(length(player_team) > 1){
    ...
    for(t in player_team){
      ...
      for(g in player_games_t){
        ...
        if( length(play) == 1){
          t_contribs = append(t_contribs, df$win_logit[play])
        }
        else if( length(play) == 0){
